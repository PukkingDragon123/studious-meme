...
// ---------------------------------------------------------------------------
const Opening = {
  // the corridor lip, the middle of the drain, and the mouth in the cistern wall
  LIP: -5968, DRAIN: -5955, MOUTH: -2996,
  on: false, cart: null, crew: [], phase: null, t: 0, taps: 0, need: 7, prompt: 0, fell: false, splashed: false, stage: null,
  slideV: 0, air: 0, rideT: 0, hintT: 0, fall: 0,

  reset() { this.on = false; this.cart = null; this.crew = []; this.phase = null; this.t = 0; this.taps = 0; this.prompt = 0; this.fell = false; this.splashed = false; this.stage = null; this.slideV = 0; this.air = 0; this.rideT = 0; this.hintT = 0; this.fall = 0; },
  ...
  loose(dt) {
    ...
    // the floor ends at the drain, and the drain has no cover on it
    if (P.x > this.LIP - 2) {
      this.fell = true; this.phase = 'drop'; this.t = 0;
      P.frozen = true; P.invuln = 999; P.vy = Math.max(P.vy, 30); P.vx = 0;
      G.banner = { text: 'DOWN', sub: '', t: 1.4, max: 1.4, color: '#c8b070' };
      SFX.breach && SFX.breach(); G.slowmo(0.45, 0.6); G.shake(4);
      for (let i = 0; i < 18; i++) G.fx.add({ type: 'drop', x: P.x + rand(-8, 8), y: P.y, vx: rand(-30, 30), vy: rand(40, 180), s: 1, color: '#8ce8a0', life: 1.4 });
    }
    this.hintT += dt;
    if (this.hintT > 14 && !this.nagged) { this.nagged = true; G.banner = { text: 'EAST', sub: 'THE FLOOR ENDS AT THE DRAIN', t: 3, max: 3, color: '#c8b070' }; }
  },
  ...
  drop(dt) {
    ...
    P.x = approach(P.x, this.DRAIN, dt * 120);
    ...
    // the rim is still under you for the first tenth of a second: do not call
    // the corridor floor a landing
    const gy = World.floorY(P.x) - 4 * P.vis;
    if (this.t > 0.18 && P.y >= gy) this.startSlide();
  },
  ...
  slide(dt) {
    ...
    const target = clamp(69 + Math.max(0, slope) * 140 + steer * 32, 48, 196);
