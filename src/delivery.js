...
const DELIVERY_PHASES = [
  { id: 'strap', at: 0.0, len: 4.2, line: 'SUBJECT SECURED' },
  { id: 'carry', at: 4.2, len: 6.4, line: 'IN TRANSIT' },
  { id: 'hatch', at: 10.6, len: 3.6, line: 'DISPOSAL HATCH 4' },
  { id: 'tip', at: 14.2, len: 2.6, line: 'RELEASE' },
  { id: 'slide', at: 16.8, len: 15.0, line: 'STORMWAY 9' },
  { id: 'splash', at: 31.8, len: 1.6, line: '' },
];
const DELIVERY_DUR = 33.4;

const Delivery = {
  begin(stage) {
    const P = G.player;
    G.deliv = {
      t: 0, stage, phase: 'strap',
  ...
  phaseAt(t) { let p = DELIVERY_PHASES[0]; for (const q of DELIVERY_PHASES) if (t >= q.at) p = q; return p; },
  local(D) { const p = this.phaseAt(D.t); return { p, u: clamp((D.t - p.at) / p.len, 0, 1) }; },
  ...
  update(raw) {
    ...
    // you are in a box and you are furious about it. It changes nothing.
    const mashing = Input.down('Space', 'KeyZ', 'KeyJ') || Input.mouse.down || (Input.touch && Input.touch.bite);
    if (p.id !== 'slide' && p.id !== 'splash') {
    ...
    if (D.t >= DELIVERY_DUR) { this.finish(); return; }
  ...
  drawLab(ctx, D, p, u) {
    ...
    const scroll = p.id === 'carry' ? D.roll : p.id === 'hatch' ? D.roll + u * 40 : 0;
    ...
    const tx = p.id === 'strap' ? W * 0.36 : p.id === 'carry' ? W * 0.36 : W * 0.5 + u * (p.id === 'tip' ? 40 : 0);
    const ty = FY - 8, tilt = p.id === 'tip' ? D.tilt : Math.sin(t * 3.1) * 0.012 * D.thrash;
    ctx.save();
    ...
    // the tank: steel base, thick glass, bolted lid, green water
    ctx.fillStyle = '#25313a'; ctx.fillRect(-40, -20, 80, 12);
    ctx.fillStyle = '#0d2420'; ctx.fillRect(-36, -74, 72, 56);
    // water
    const wl = -74 + 8;
    ctx.fillStyle = '#1d5a4a'; ctx.fillRect(-36, wl, 72, 50);
    ctx.fillStyle = '#2a7a62'; ctx.fillRect(-36, wl, 72, 3);
    ...
    // this uses the preview rig, which is solved in local space.
    {
      ...
      ctx.save();
      ctx.beginPath(); ctx.rect(-36, -74, 72, 56); ctx.clip();
      ctx.translate(sw * 0.35, -34 + Math.cos(t * 5) * 2);
      ctx.rotate(sw * 0.05);
      this.croc(ctx, 0.62, 5 + D.thrash * 7);
      ctx.restore();
    }
    // glass: rim light, a crack that never grows, condensation
    ctx.globalAlpha = 0.26; ctx.fillStyle = '#cfeee6'; ctx.fillRect(-32, -70, 4, 48); ctx.globalAlpha = 1;
    if (D.bang > 0) { ctx.globalAlpha = D.bang * 3; ctx.fillStyle = '#dffdf4'; ctx.fillRect(-36, -74, 72, 56); ctx.globalAlpha = 1; }
    ctx.strokeStyle = 'rgba(210,240,235,0.5)'; ctx.lineWidth = 1; ctx.strokeRect(-36.5, -74.5, 73, 57);
    // lid, bolted
    ctx.fillStyle = '#2e3a42'; ctx.fillRect(-40, -82, 80, 9);
    ctx.fillStyle = '#44525c'; ctx.fillRect(-40, -82, 80, 2);
    for (let k = 0; k < 6; k++) { ctx.fillStyle = '#6a7a84'; ctx.fillRect(-35 + k * 14, -80, 4, 4); }
    // straps over the lid
    ctx.fillStyle = '#3a2e1e'; ctx.fillRect(-42, -66, 84, 5); ctx.fillRect(-42, -40, 84, 5);
    ctx.fillStyle = '#8a7a4a'; ctx.fillRect(-8, -68, 16, 9); ctx.fillRect(-8, -42, 16, 9);
    // stencil
    Font.draw(ctx, 'LIVE', 0, -58, { color: '#cfe0d8', align: 'center' });
    Font.draw(ctx, 'SPECIMEN', 0, -50, { color: '#6f8f88', align: 'center' });
    ctx.restore();

    // ---- the two who are pushing it -------------------------------------
    if (p.id !== 'tip' || u < 0.5) {
      for (const [ox, f] of [[-72, 1], [-104, 1]]) {
        const px2 = tx + ox, step = p.id === 'carry' ? Math.sin(D.roll * 0.08 + (ox < -80 ? 1.4 : 0)) : 0;
  ...
  drawSlide(ctx, D, u) {
    ...
    const wallAt = k => lerp(16, 330, k * k);          // k: 0 far, 1 near
    ...
    const secs = Math.max(0, Math.ceil((DELIVERY_PHASES[4].at + DELIVERY_PHASES[4].len) - D.t));
  ...
  finish() {
    ...
    G.shake(8);
    G.banner = { text: 'IN THE SYSTEM', sub: 'THERE IS NO DOOR ON THE OTHER END', t: 4, max: 4, color: '#8ab820' };
  },
};
